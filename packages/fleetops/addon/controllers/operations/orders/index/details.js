...
export default class OperationsOrdersIndexDetailsController extends Controller {
    ...
    async syncRoutingControl() {
        ...
        this.routingControl = await this.mapManager.addRoutingControl(this.routeMarkerWaypoints, this.routingControlOptions);
    ...
    async setupDetailsSession() {
        ...
        await this.setupRealtime().catch((error) => {
            debug(`[order details] setupRealtime failed: ${error?.message ?? error}`);
        });
