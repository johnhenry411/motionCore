...
class TrackingIntelligenceService
{
    ...
    protected function buildResult(TrackingContext $context, TrackingProviderResult $providerResult, TrackingOptions $options): array
    {
        ...
        return [
            ...
            // Legacy flat fields kept for backward compatibility with clients (e.g. driver app)
            // built against the pre-TrackingIntelligenceService `OrderTracker::toArray()` shape.
            'driver_current_location'           => $this->pointToGeoJson($context->driverLocation),
            'progress_percentage'               => $progress['percentage'],
            'total_distance'                    => $providerResult->distanceMeters,
            'completed_distance'                => $progress['completed_distance_m'],
            'current_destination_eta'           => $activeStopSeconds,
            'completion_eta'                    => $completionSeconds,
            'estimated_completion_time'          => $this->addSeconds($now, $completionSeconds),
            'estimated_completion_time_formatted' => $completionSeconds === null ? null : $now->copy()->addSeconds((int) round($completionSeconds))->format('M jS, Y H:i'),
            'start_time'                          => optional($context->order->started_at)->toISOString(),
            'completion_time'                     => optional($context->order->completed_at)->toISOString(),
            'current_destination'                 => $context->activeStop?->toArray(),
            'next_destination'                    => $context->nextStop?->toArray(),
            'first_waypoint_completed'            => $context->completedStops->count() > 0,
            'last_waypoint_completed'             => $progress['remaining_stops'] === 0,
        ];
    }
